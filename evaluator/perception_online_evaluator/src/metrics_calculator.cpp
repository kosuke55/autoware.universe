...
//     Stat<double> stat;
//     stat.add(
//       static_cast<double>(static_cast<double>(count) /
//       static_cast<double>(objects_count_frame_)));
...
MetricStatMap MetricsCalculator::calcObjectsCountMetrics() const
{
  ...
  for (const auto & [label, count] : historical_detection_count_map_) {
    ...
    // std::cout << "historical_objects_count_" << convertLabelToString(label) << ": "
    //           << static_cast<double>(count) / static_cast<double>(objects_count_frame_)
    //           << " total: " << count << " frame: " << objects_count_frame_ << std::endl;
  }

  const double frame_count = static_cast<double>(detection_count_vector_.size());
  for (const auto & [detection_count_map, _] : detection_count_vector_) {
    for (const auto & [label, count] : detection_count_map) {
      metric_stat_map["interval_objects_count_" + convertLabelToString(label)].add(
        static_cast<double>(count) / frame_count);
      // std::cout << "interval_objects_count_" << convertLabelToString(label) << ": "
      //           << static_cast<double>(count) / frame_count << std::endl;
    }
  }
...
// const Point ego_pos_from_ego = inverseTransformPoint(
    //   object.kinematics.initial_pose_with_covariance.pose.position, ego_pose_.value());    
