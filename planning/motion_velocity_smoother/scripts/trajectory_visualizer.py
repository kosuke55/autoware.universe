...
class TrajectoryVisualizer(Node):
    def __init__(self):
        ...
        self.update_traj_a = False
        self.update_traj_b = False
        self.update_traj_c = False
        ...
        self.ts1 = message_filters.ApproximateTimeSynchronizer(
            [self.sub1, self.sub2, self.sub3, self.sub4, self.sub5, self.suba, self.subb, self.subc], 30, 0.5
        )
    ...
    def CallbackMotionVelOptTraj(self, cmd1, cmd2, cmd3, cmd4, cmd5, cmd6, cmd7, cmd8):
    ...
    def CallBackA(self, cmd):
        self.trajectory_a = cmd
        self.update_traj_a = True

    def CallBackB(self, cmd):
        self.trajectory_b = cmd
        self.update_traj_b = True

    def CallBackC(self, cmd):
        self.trajectory_c = cmd
        self.update_traj_c = True
    ...
    def setPlotTrajectoryVelocity(self):
        ...
        (self.ima, ) = self.ax1.plot([], [], label="backward_filtered_trajectory", marker="", ls="--")
        (self.imb, ) = self.ax1.plot([], [], label="forward_filtered_trajectory", marker="", ls="--")
        (self.imc, ) = self.ax1.plot([], [], label="merged_filtered_trajectory", marker="", ls="--")
    ...
    def plotTrajectoryVelocity(self, data):
        ...
        trajectory_c = self.trajectory_c
        ...
        if self.update_traj_a:
            x = self.CalcArcLength(trajectory_a)
            y = self.ToVelList(trajectory_a)
            self.ima.set_data(x, y)
            self.update_traj_a = False

        if self.update_traj_b:
            x = self.CalcArcLength(trajectory_b)
            y = self.ToVelList(trajectory_b)
            self.imb.set_data(x, y)
            self.update_traj_b = False

        if self.update_traj_c:
            x = self.CalcArcLength(trajectory_c)
            y = self.ToVelList(trajectory_c)
            self.imc.set_data(x, y)
            self.update_traj_c = False
