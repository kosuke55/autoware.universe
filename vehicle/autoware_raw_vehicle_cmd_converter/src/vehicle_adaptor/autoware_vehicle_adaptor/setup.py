...
if not SKIP_PRE_INSTALL_FLAG:
    ...
    build_inputs_prediction_command += "autoware_vehicle_adaptor/src/inputs_prediction.cpp "
    build_inputs_prediction_command += (
        "-o autoware_vehicle_adaptor/src/inputs_prediction$(python3-config --extension-suffix) "
    )
    build_inputs_prediction_command += "-lrt -I$(pwd)/autoware_vehicle_adaptor/include -I/usr/include/eigen3"
    print(build_inputs_prediction_command)
    subprocess.run(build_inputs_prediction_command, shell=True)

utils_so_path = "src/" + glob.glob("autoware_vehicle_adaptor/src/utils.*.so")[0].split("/")[-1]
...
setup(
    ...
    package_data={
        "autoware_vehicle_adaptor": ["package_path.json", utils_so_path, actuation_map_so_path, nominal_ilqr_so_path, inputs_prediction_so_path],
    },
)
